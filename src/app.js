...
globalThis.addEventListener?.('error', (event) => renderError(event.error || event.message));
globalThis.addEventListener?.('unhandledrejection', (event) => renderError(event.reason));

if (!app) throw new Error('Root element #app не найден в index.html');
...
function render() {
  const view = views[state.section] || views.dashboard;
  app.innerHTML = `
    ...
    <main class="shell">
      ${header()}
      <section class="view">${view()}</section>
    </main>`;
  bindGlobal();
}
...
function titleFor(section) {
  return ({ dashboard: 'Центр управления жизнью', tasks: 'Система задач', schedule: 'Планировщик дня', memory: 'Память и журнал', analytics: 'Поведенческая аналитика', insights: 'AI-инсайты и отчёты', ai: 'Универсальный AI-вход', settings: 'Настройки и резервная память' })[section] || 'Центр управления жизнью';
}
...
const views = {
  ...
  settings: () => `<div class="grid two"><article class="card glass"><h3>AI ключ для браузера</h3><p>Не хранится в коде: только локально в IndexedDB. Для автономии используйте GitHub Secrets.</p><input id="groq" type="password" placeholder="Groq API key" value="${escapeHtml(state.settings.groqApiKey || '')}"/><button data-action="save-key">Сохранить</button></article><article class="card glass"><h3>GitHub backup</h3><input id="repo" placeholder="owner/repo" value="${escapeHtml(state.settings.githubRepo || '')}"/><input id="token" type="password" placeholder="Fine-grained token Contents RW" value="${escapeHtml(state.settings.githubToken || '')}"/><button data-action="save-github">Сохранить sync</button><button data-action="push-github">Выгрузить snapshot</button></article></div><article class="card glass"><h3>Экспорт / импорт</h3><button data-action="export">Скачать JSON</button><label class="file">Импорт JSON<input type="file" id="import" accept="application/json"/></label></article>`
};
...
function taskList(tasks) { return `<div class="list">${tasks.map((task) => `<div class="item"><button data-done="${task.id}" class="check ${task.status === 'done' ? 'on' : ''}"></button><div><b>${escapeHtml(task.title)}</b><small>${task.priority} · ${task.estimate || 30} мин · ${task.deadline || 'без дедлайна'}</small>${task.subtasks?.length ? `<em>${escapeHtml(task.subtasks.join(' → '))}</em>` : ''}</div><button data-delete-task="${task.id}">×</button></div>`).join('') || '<p class="muted">Пока пусто.</p>'}</div>`; }
...
function bindGlobal() {
  ...
  document.querySelectorAll('[data-action]').forEach((button) => button.addEventListener('click', () => Promise.resolve(actions[button.dataset.action]?.()).catch(renderError)));
  document.querySelector('#import')?.addEventListener('change', importFile);
}

async function saveTask(event) { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); await put('tasks', { id: uid('task'), ...data, estimate: Number(data.estimate || 30), status: 'active', subtasks: [], createdAt: new Date().toISOString() }); await refresh(); }
async function saveNote(event) { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); await put('notes', { id: uid('note'), ...data, createdAt: new Date().toISOString() }); await refresh(); }
async function saveEvent(event) { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); await put('events', { id: uid('event'), ...data, type: 'manual', createdAt: new Date().toISOString() }); await refresh(); }
async function toggleTask(id) { const task = state.tasks.find((item) => item.id === id); if (!task) return; await put('tasks', { ...task, status: task.status === 'done' ? 'active' : 'done', completedAt: task.status === 'done' ? null : new Date().toISOString() }); await refresh(); }
async function sendAI(event) { event.preventDefault(); const prompt = new FormData(event.target).get('prompt'); if (!prompt) return; await put('messages', { id: uid('msg'), role: 'user', content: prompt, createdAt: new Date().toISOString() }); state.aiBusy = true; await refresh(); let content; try { content = await askAI({ apiKey: state.settings.groqApiKey, prompt, memories: state.memories, tasks: state.tasks, notes: state.notes, events: state.events }); } catch { content = localCoach(prompt, load(), analytics()); } await put('messages', { id: uid('msg'), role: 'assistant', content, createdAt: new Date().toISOString() }); state.aiBusy = false; await refresh(); }

const actions = {
  'save-key': async () => { await setSetting('groqApiKey', document.querySelector('#groq').value.trim()); await refresh(); },
  'save-github': async () => { await setSetting('githubRepo', document.querySelector('#repo').value.trim()); await setSetting('githubToken', document.querySelector('#token').value.trim()); await refresh(); },
  'export': async () => download('megabot-memory.json', JSON.stringify(await exportSnapshot(), null, 2)),
  'push-github': pushGithub,
  'auto-plan': async () => { for (const event of buildDayPlan(todayTasks(), [])) await put('events', { ...event, id: uid('event'), createdAt: new Date().toISOString() }); await refresh(); },
  'breakdown': async () => { const task = todayTasks()[0]; if (!task) return; await put('tasks', { ...task, subtasks: ['Определи критерий готовности', 'Сделай первый 10-минутный шаг', 'Проверь результат и следующий блок'] }); await refresh(); },
  'daily-local': async () => { const a = analytics(); await put('summaries', { id: uid('sum'), title: `Отчёт ${todayKey()}`, summary: `${a.pattern} Выполнение ${a.completion}%, энергия ${a.avgEnergy}%. Рекомендация: завтра начни с самой дорогой когнитивной задачи.`, createdAt: new Date().toISOString() }); await refresh(); }
};
async function importFile(event) { const file = event.target.files[0]; if (!file) return; await importSnapshot(JSON.parse(await file.text())); await refresh(); }
